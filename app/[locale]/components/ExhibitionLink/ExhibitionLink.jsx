    ...
    return(
        ...
        { exhibition.images.map((each, index)=> index< 1 &&
          <div key={index} style={{display:'flex',justifyContent:'center'}}>
            <img style={{width:'80%'}} alt='imagen exhibicion' className={styleExhibition.cardImageExhibition} key={index} srcSet={each.replace('http','https')} src={each.replace('http','https')} /> 
              </div>
        )}
      </Link>
    )
